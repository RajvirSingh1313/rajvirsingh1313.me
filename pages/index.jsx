...
export default function Home() {
  return (
    <main id="main" className="HomeHero px-10 xl:px-24 sm:py-10 text-2xl h-5/6">
      <div className="flex w-full h-full flex-col sm:flex-row">
        ...
        <div className="pt-10 sm:pt-0 sm:w-1/2 flex flex-col items-center justify-center xl:px-10 space-y-6 sm:space-y-14 px-4">
          <Link href="/about">
            <div className="w-full lg:w-3/4 bg-lightBlue xl:px-16 py-8 xl:py-10 flex justify-between items-center rounded-xl shadow-lg hover:shadow-2xl hover:bg-midBlue/30 border-2 border-lightBlue hover:border-activeBlue hover:cursor-pointer">
          ...
          <div className="w-full lg:w-3/4 flex justify-center lg:justify-end">
            <Link href="/talk">
              <button className="px-10 py-4 bg-activeBlue rounded-full text-darkBlue font-bold text-xl">Lets Talk</button>
            </Link>
          </div>
        </div>
      </div>

      <a rel="noreferrer" target="_blank" href="https://in.linkedin.com/in/deninpaul"><p className="py-10 sm:py-0 pl-10 font-normal text-xl">Designed By <span className="text-activeBlue hover:underline hover:text-white hover:bg-activeBlue cursor-pointer">Denin Paul</span></p></a>
    </main>
  );
}
